...
```{r setting time origin}
setRefDate("1950/01/01")
constant.define("asp",0)
```
...
intaros.save.environ(long_lim = long_lim, lat_lim = lat_lim,
                     flag_file = FALSE, 
                     image_dir = file.path(dir.name, image.name))
```
...
years      = subyears = get_db_sequence_year(db0)
trimesters = subtrims = seq(1,4)
colyears   = rg.colors(length(years))
cat(build_title("The dataset period is:",
                time2date(get_db_limits_time(db0))))
```
...
display_var(db0, var = var, colors = colors.temp, title = var, 
            filename = var)
```
...
display_var(db0, var = var, colors = colors.temp, title = filename,
            filename = filename)
...
long_lim = c(-2,10)
lat_lim  = c(56,62)
dbloc    = apply_sel(db0, long_lim = long_lim, lat_lim = lat_lim, 
                     compress = TRUE)
intaros.save.environ(long_lim = long_lim, lat_lim  = lat_lim)
```
...
dbreg = aggregate_depth(dbloc, depth0 = 0, ddepth = 10, ndepth = 10,
                        flag.center = TRUE)
...
# Display means
plot (years,rres$means,type="b",
      main=paste("Mean of",var,"at",depth,"m"),pch=15,lty=1,col=colyears)
lines(years,cres$means,type="b",pch=19,lty=2,col=colyears)
legend("topleft",legend=c("Initial","Aggregated"),lty=c(2,1),pch=c(19,15))

# Display variances
plot (years,rres$vars ,type="b",
      main=paste("Variance of",var,"at",depth,"m"),
...
# Display statistics
plot(res$count,res$depths,type="b",
     main=paste("Number of samples for",var),pch=19,
...
plot(dbg, title = build_title_db_time(dbg,var),  
     xlab = "Time", ylab = var)
```
...
plot(dbg, title=build_title_db_depth(dbg,var), 
     xlab="Depth", ylab = var)
```
...
dbr       = apply_sel(dbreg, depth_lim = depth_lim, date_lim = date_lim,
                      compress = TRUE)
...
db.stat.print(dbp,names="Xvalid*",funs=c("num","mean","var"),
...
# Interpolate in 3-D
dbg = interpolate_3D(dbr, var, mesh = 0.1, depth0 = depth0, ndepth = 10,
                     ddepth = ddepth,
                     vario_lag = vario_lag, vario_nlag = vario_nlag, 
                     struct = c(1,3,5,12), 
                     dirvect = NA, verbose=FALSE, draw.model=TRUE,
                     pos.legend=1)
